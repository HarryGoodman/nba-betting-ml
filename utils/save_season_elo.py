...
def elo_formula(game, elo, k=K_FACTOR, m=M):
    ...
    opponent = game["MATCHUP"].split(" ")[
        -1
    ]  # Extract the opponent team from the matchup
    date = game["GAME_DATE"]
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Compute Elo ratings for an NBA season and save as Parquet."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="NBA season to compute Elo ratings for (e.g., '2023-24').",
    )
    parser.add_argument(
        "--input-type",
        required=True,
        choices=["local", "s3"],
        help="Where to load the Parquet file from: 'local' or 's3'.",
    )
    parser.add_argument(
        "--input-filepath",
        required=True,
        type=str,
        help="Input file path (local file path or S3 key).",
    )
    ...
    parser.add_argument(
        "--output-filepath",
        type=str,
        help="Output file path for saving Parquet file (required if output type is 'local').",
    )

    parser.add_argument(
        "--bucket-name",
        type=str,
        help="S3 bucket name (required if using S3 for saving/loading).",
    )
    parser.add_argument(
        "--s3-key",
        type=str,
        help="S3 key (path) for saving/loading the Parquet file in S3 (required if input/output type is 's3').",
    )
