...
def fetch_all_players_from_rosters(season):
    ...
    try:
        ...
        for team_id in team_ids:
            ...
            print(
                f"Fetched {len(team_roster['PLAYER_ID'])} players from team ID {team_id}"
            )
...
def fetch_player_stats_for_season(player_ids, season):
    ...
    try:
        ...
        all_players_stats_df["GAME_DATE"] = (
            all_players_stats_df["GAME_DATE"].str.strip().str.upper()
        )
        all_players_stats_df["GAME_DATE"] = pd.to_datetime(
            all_players_stats_df["GAME_DATE"], format="%b %d, %Y"
        )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch NBA player stats and save as Parquet."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="NBA season to fetch (e.g., '2023-24').",
    )
    parser.add_argument(
        "--save-type",
        required=True,
        choices=["local", "s3"],
        help="Where to save the Parquet file: 'local' or 's3'.",
    )
    ...
    parser.add_argument(
        "--filepath",
        type=str,
        help="Local file path for saving Parquet file (required if save-type is 'local').",
    )
    ...
    parser.add_argument(
        "--bucket-name",
        type=str,
        help="S3 bucket name (required if save-type is 's3').",
    )
    parser.add_argument(
        "--s3-key",
        type=str,
        help="S3 key (path) for saving the Parquet file in S3 (required if save-type is 's3').",
    )
