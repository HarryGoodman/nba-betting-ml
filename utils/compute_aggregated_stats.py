...
def get_team_stats_average(team_df, team, game_date, lag):
    ...
    team_stats = team_df[
        (team_df["TEAM_ABBREVIATION"] == team) & (team_df["GAME_DATE"] < game_date)
    ]
    if team_stats.empty:
...
def get_player_stats_top_n_average(
    ...
    player_stats = player_df[
        (player_df["MATCHUP"].str.contains(team)) & (player_df["GAME_DATE"] < game_date)
    ]
    ...
    top_players = (
        last_n_games.groupby("Player_ID")["MIN"]
        .sum()
        .nlargest(n_players)
        .index.tolist()
    )

    top_player_stats = last_n_games[
        last_n_games["Player_ID"].isin(top_players[: len(last_n_games)])
    ]

    player_names = {
        player_id: player_mapping.get(player_id, f"Player_{player_id}")
        for player_id in top_players[: len(top_player_stats)]
    }

    avg_player_stats = (
        top_player_stats.groupby("Player_ID")[PLAYER_STATS_COLUMNS[1:]]
        .mean()
        .values.flatten()
    )
    ...
    if padding_needed > 0:
        avg_player_stats = np.concatenate(
            [
                avg_player_stats,
                np.zeros((padding_needed, len(PLAYER_STATS_COLUMNS[1:]))),
            ],
            axis=None,
        )
    ...
    return (
        avg_player_stats,
        player_columns,
        top_players[: len(top_player_stats)],
        player_names,
    )
...
def compute_stats(
    ...
    avg_player_stats, player_columns, top_players, player_names = (
        get_player_stats_top_n_average(
            player_df,
            team,
            game_date,
            lag,
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Compute team and player stats along with Elo for a given date, team, and lag."
    )

    parser.add_argument(
        "--elo-path", required=True, type=str, help="Path to the Elo data parquet file."
    )
    parser.add_argument(
        "--team-path",
        required=True,
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to the player stats parquet file.",
    )
    parser.add_argument(
        "--team",
        required=True,
    ...
    parser.add_argument(
        "--date", required=True, type=str, help="Game date in YYYY-MM-DD format."
    )
    parser.add_argument(
        "--lag",
        required=True,
